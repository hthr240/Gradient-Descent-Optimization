...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        logistic = LogisticModule(weights)

        if self.penalty_ == "none":
            module = logistic
        else:
            # Create regularization module with shared weights
            if self.penalty_ == "l1":
                regularization = L1(weights[1:] if self.include_intercept_ else weights)
            elif self.penalty_ == "l2":
                regularization = L2(weights[1:] if self.include_intercept_ else weights)
            else:  
                raise ValueError("Unsupported penalty type")
        
            module = RegularizedModule(
                fidelity_module=logistic,
    ...
    def _predict(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        if self.include_intercept_:
        # Add intercept only if not already present
            if X.shape[1] == len(self.coefs_) - 1:
                X = np.c_[np.ones(X.shape[0]), X]
