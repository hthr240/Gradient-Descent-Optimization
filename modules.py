...
class RegularizedModule(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        if X is None or y is None:
            raise ValueError("Missing required keyword arguments: 'X' and 'y'")
        return self.fidelity_module_.compute_output(X, y) + self.lam_ * self.regularization_module_.compute_output()
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        if X is None or y is None:
            raise ValueError("Missing required keyword arguments: 'X' and 'y'")
        fidelity_grad = self.fidelity_module_.compute_jacobian(X, y).ravel()
        regularization_grad = self.regularization_module_.compute_jacobian().ravel()

        if self.include_intercept_:
            reg_grad_full = np.concatenate(([0], regularization_grad)) 
        else:
            reg_grad_full = regularization_grad

        return fidelity_grad + self.lam_ * reg_grad_full
